...
// 	let len = ring.length, klen = key.length, index = 0, cur = 0, res = Number.MAX_SAFE_INTEGER, m = {};
// 	for(let i=0;i<len;i++){
// 		if(ring[i] in m){
// 			m[ring[i]].push(i);
// 		}else{
...
// 		}
// 	}
// 	let dfs = function(index,cur,count){
// 		console.log(index,cur,count)
// 		if(count > res)return;
// 		if(index >= klen){
...
// 		}else{
// 			for(let i=0;i<m[key[index]].length;i++){
// 				let min = Math.min((cur+len-m[key[index]][i])%len,(m[key[index]][i]+len-cur)%len);
// 				dfs(index+1,m[key[index]][i],count+min+1);
// 			}
// 		}
// 	}
// 	dfs(0,0,0);
...
console.log(findRotateSteps("d","d"))